...
def find_path_to_ZZ(maze, connections):
    ...
    while queue:
        steps, portal_id = heapq.heappop(queue)

        if portal_id in visited:
            continue

        if portal_id == "ZZ":
            return steps

        visited.add(portal_id)

        for portal, distance in connections[portal_id]:
            new_steps = steps + distance + 1

            if portal.id == "ZZ":
                heapq.heappush(queue, (new_steps - 1, portal.id))
                continue
            ...
            if new_portal_id not in visited:
                heapq.heappush(queue, (new_steps, new_portal_id))
